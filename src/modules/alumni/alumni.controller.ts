...
export const alumniController = {
  list: async (req: Request, res: Response) => {
    ...
    // Default: only show approved alumni for public (no auth header)
    if (!query.approved && !req.headers.authorization) {
      query.approved = "true";
    }
  ...
  register: async (req: Request, res: Response) => {
    ...
    try {
      const result = await alumniService.register(parsed.data);
      return res.status(201).json({
        status: 201,
        message: "success",
        data: result,
