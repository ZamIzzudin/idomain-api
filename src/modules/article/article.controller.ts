...
export const articleController = {
  list: async (req: Request, res: Response) => {
    ...
    const result = await articleService.list(parsed.data);
  ...
  filterOptions: async (_req: Request, res: Response) => {
    const options = await articleService.filterOptions();
  ...
  update: async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const article = await articleService.update(Number(id), payload);
      return res.json({ status: 200, message: "success", data: article });
    } catch (error: any) {
      return res.status(400).json({
        status: 400,
  ...
  remove: async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      return res.json({
        status: 200,
        message: `Successfully removed article ${id}`,
      });
    } catch (error: any) {
      return res.status(400).json({
        status: 400,
